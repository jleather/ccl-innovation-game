...
socket.on("admin:missions", (missions) => {
  ...
  document.querySelectorAll(".mission-card[data-code]").forEach(card => {
    card.onclick = () => {
      document.querySelectorAll(".mission-card[data-code]").forEach(c=>c.classList.remove("selected"));
      card.classList.add("selected");
      selectedMissionCode = card.dataset.code;
      socket.emit("admin:selectMission", selectedMissionCode);
      document.getElementById("trackSelector").classList.toggle("hidden", selectedMissionCode !== "CA");
    };
  });
  document.querySelectorAll("[data-track]").forEach(card => {
    card.onclick = () => {
      document.querySelectorAll("[data-track]").forEach(c=>c.classList.remove("selected"));
      card.classList.add("selected");
      socket.emit("admin:selectTrack", card.dataset.track);
    };
  });
});
...
socket.on("state:update", (state) => {
  ...
  if (state.selectedTrack) selectedTrack = state.selectedTrack;
  if (!isAdmin) {
...
const raceSnapshotListeners = new Set();
socket.on("race:snapshot", (racers) => { for (const fn of raceSnapshotListeners) fn(racers); });
...
const missionApi = {
  ...
  onControl: (id, handler) => { const el = document.getElementById(id); if (el) el.onclick = () => { if (!gamePaused) handler(); }; },
  get selectedTrack() { return selectedTrack; },
  get myAlias() { return myAlias; },
  get myColor() { return myColor; },
  sendRaceUpdate: (data) => socket.emit("player:raceUpdate", data),
  onRaceSnapshot: (fn) => raceSnapshotListeners.add(fn),
  offRaceSnapshot: (fn) => raceSnapshotListeners.delete(fn)
};
...
socket.on("match:start", ({ mission, endsAt }) => {
  ...
  document.getElementById("hudInstructions").textContent = "📖 " + (MISSIONS_INFO[mission].instrucciones || "");
...
function loop() {
  ...
  if (activeMission) {
    if (!gamePaused) activeMission.update(dt);
...
canvas.addEventListener("click", (e) => {
  if (gamePaused || !activeMission || !activeMission.onClick) return;
...
canvas.addEventListener("mousemove", (e) => {
  if (gamePaused || !activeMission || !activeMission.onMouseMove) return;
  const rect = canvas.getBoundingClientRect();
  const x = (e.clientX - rect.left) * (canvas.width / rect.width);
  const y = (e.clientY - rect.top) * (canvas.height / rect.height);
  activeMission.onMouseMove(x, y);
});
window.addEventListener("keydown", (e) => { if (!gamePaused && activeMission && activeMission.onKey) activeMission.onKey(e.key, true); });
window.addEventListener("keyup", (e) => { if (!gamePaused && activeMission && activeMission.onKey) activeMission.onKey(e.key, false); });
...
// ---- POPUP DE PREGUNTAS (llega SOLO a este jugador — ni otros jugadores ni el admin lo ven) ----
  ...
  options.forEach((opt, i) => {
    ...
    btn.onclick = () => {
      ...
      setTimeout(() => {
        ...
        gamePaused = false;
        lastTime = performance.now(); // evita un salto grande de dt tras la pausa
      }, 600);
    };
...
// ---- FIN DE PARTIDA / RANKING (llega a TODOS: jugadores y administrador) ----
socket.on("match:end", ({ ranking, mission }) => {
  if (rafId) cancelAnimationFrame(rafId);
  if (adminTimerInterval) clearInterval(adminTimerInterval);
  gamePaused = false;
  document.getElementById("questionOverlay").classList.add("hidden");
  ...
  activeMission = null;
  showScene("ranking");
  document.querySelector("#scene-ranking h2").textContent = `🏆 Ranking Final — ${mission ? MISSIONS_INFO[mission].nombre : ""}`;
  const list = document.getElementById("rankingList");
  list.innerHTML = ranking.map((p, i) => {
    const cls = i===0?"top1":i===1?"top2":i===2?"top3":"";
    return `<div class="rank-row ${cls}"><span><span class="rank-pos">#${i+1}</span>${escapeHtml(p.alias)}</span><span>${p.score} pts</span></div>`;
  }).join("") || "<p class='subtitle'>Nadie anotó puntos en esta partida.</p>";
});
